...
const SEARCH_STATUS = {
  ...
  function filterRowsIndexesWithAllSearchTerms(arr1, arr2, arr3) {
    // return empty if all search term are present but even one array is empty
    if (isAllSearchTermFilled) {
      if (arr1.length < 1 || arr2.length < 1 || arr2.length < 1) return [];
    }
    ...
    if (arr.length === 3) {
      ...
      arr[0].forEach((val) => {
        // console.log("Arr1 item: ", val);
        // console.log("arr2", arr[1]);
        // console.log("arr3", arr[2]);
