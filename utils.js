...
const SEARCH_STATUS = {
    ...
    // OR_SEARCH_SUCCESFULL: "Search Was Successfull!",
    SEARCH_FAILURE: "No items we found in the given given critera!",
    //OR_SEARCH_FAILURE: "No items we found in the given given critera!"
  }
  ...
  function searchSheetByColumn(sheet, rangeNotation, searchVal) {
    ...
    const allIndexes = data.map((val, index) => ({ val, index }))
      .filter(({ val, index }) => rangeNotation === quantityRangeNotation ? val == searchVal : val.toLowerCase().includes(searchVal.toLowerCase()))
   ...
   */
    ...
    // skip looping for arrays that are empty
    const arr = [arr1, arr2, arr3];
    const matchingIndexes = [];
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].length < 1) continue; // this takes care of case when not all keywords is non-empty 
      arr[i].forEach((val) => {
